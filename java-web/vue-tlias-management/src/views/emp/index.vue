...
import { getEmpList,getEmpById,updateEmp,delEmpById ,addApi} from '@/api/emp'
...
const search= async () => {
  ...
  const result = await getEmpList(searchEmp.value.name, searchEmp.value.gender, searchEmp.value.begin, searchEmp.value.end, currentPage.value, pageSize.value);
  if(result.code){
...
const deleteById = (id) => {
  ...
  ElMessageBox.confirm('您确认删除该员工吗?','提示',
    ...
    const result = await delEmpById(id);
    if(result.code){
...
const deleteByIds = () => {
  ...
  ElMessageBox.confirm('您确认删除该员工吗?','提示',
    ...
    if(selectedIds.value && selectedIds.value.length > 0){
      const result = await delEmpById(selectedIds.value);
      if(result.code){
        ElMessage.success('删除成功');
        search();
      }else{
        ...
        <el-row :gutter="3" v-for="(item, index) in employee.exprList" :key="index">
          <el-col :span="10">
            <el-form-item size="small" label="时间" label-width="80px">
              <el-date-picker type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" format="YYYY-MM-DD" value-format="YYYY-MM-DD"  v-model="item.exprDate"></el-date-picker>
            </el-form-item>
          </el-col>
