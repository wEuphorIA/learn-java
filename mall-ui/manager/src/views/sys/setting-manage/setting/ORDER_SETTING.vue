<template>
  <div class="layout">
    <Form ref="formValidate" :label-width="150" label-position="right" :model="formValidate" :rules="ruleValidate">
      ...
      <FormItem label="已完成订单允许退单" prop="autoAfterSaleComplete">
        <Input type='number' v-model="formValidate.autoAfterSaleComplete">
        <span slot="append">天</span>
        </Input>
        <span class="desc">订单完成后，多少天内允许退单</span>
      </FormItem>
      <FormItem label="待审核退单自动审核" prop="autoAfterSaleReview">
        <Input type='number' v-model="formValidate.autoAfterSaleReview">
        <span slot="append">天</span>
        </Input>
        <span class="desc">待审核退单自动审核</span>
      </FormItem>
      <FormItem label="售后自动取消" prop="autoCancelAfterSale">
        <Input type='number' v-model="formValidate.autoCancelAfterSale">
        <span slot="append">天</span>
        </Input>
        <span class="desc">售后自动取消</span>
      </FormItem>
