  ...
  <div class="contentBox">
    ...
    >
      <Form ref="form" :model="form" :label-width="70" :rules="formValidate">
        ...
        <FormItem label="角色：" prop="rolesList">
          <Select v-model="form.rolesList" multiple>
            <Option
...
export default {
  ...
  methods: {
    ...
    edit(v) {
      ...
      let selectRolesId = [];
      if (this.form.rolesList) {
        this.form.rolesList.forEach(function (e) {
          selectRolesId.push(e.id);
        });
      }
      this.form.rolesList = selectRolesId;
      this.userModalVisible = true;
    },
    handleEnableOpen(item) {
      this.userId = item.id;
      this.userText = item.username;
    ...
    handleEnableSub(v) {
      let params = {
        status: this.enableOperations,
      };
      enableUser(this.userId, params).then((res) => {
        this.$Modal.remove();
        if (res.success) {
          this.$Message.success("操作成功");
