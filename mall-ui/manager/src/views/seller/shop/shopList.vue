...
<script>
// 关闭弹窗
import ColseDialog from "@/components/CloseDialog/index.vue";
import {
  getShopListData,
  disableShop,
  enableBrand,
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          tooltip: true,
          render: (h, params) => {
            return h("span", {}, params.row.storeAddressPath || "暂未填写");
          },
        },
        {
          title: "是否自营",
          ...
          width: 120,
          render: (h, params) => {
            return h("span", params.row.selfOperated ? "自营" : "非自营");
          },
        },
            ...
            if (params.row.storeDisable == "OPEN") {
              enableOrDisable = h(
                "span",
                {
                  ...
                  on: {
                    click: () => {
                      this.handleOpen(params.row, "close");
                    },
                  },
                },
                "关闭"
              ...
              enableOrDisable = h(
                "span",
                {
                  ...
                  on: {
                    click: () => {
                      this.handleOpen(params.row, "open");
                    },
                  },
                },
                "开启"
  ...
  methods: {
    ...
    handleOpen(item, type) {
      this.goodId = item.id;
      this.goodType = type;
      this.goodsText=item.storeName
      this.closeVisible = true;
    },
    // 关闭/开启店铺
    handleSubmit(v) {
      if (this.goodType === "close") {
        disableShop(this.goodId).then((res) => {
          this.$Modal.remove();
          if (res.success) {
