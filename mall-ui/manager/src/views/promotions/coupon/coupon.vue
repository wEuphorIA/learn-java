<template>
  <div class="contentBox">
    ...
    <div class="boxBg boxPad m24">
      <div class="export" v-if="getType !== 'ACTIVITY'">
        <Button @click="add" type="primary">添加优惠券</Button>
        <Button @click="delAll" class="ml_20">批量关闭</Button>
        <Button @click="receivePage()" class="ml_20">优惠券领取记录</Button>
      </div>
      <Table
        v-if="refreshTable"
        :loading="loading"
      ...
      >
        <template slot-scope="{ row }" slot="action">
          ...
          <span v-else class="fontCol" @click="see(row, 'onlyView')"
            >查看
          </span>
          <span
            class="fontCol"
          ...
          <!--&gt;删除-->
          <!--</Button>-->
          <span class="fontCol" @click="receivePage(row.id)">领取记录 </span>
        </template>
      </Table>
