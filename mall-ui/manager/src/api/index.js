...
let commonUrl = (process.env.NODE_ENV === 'development' ? BASE.API_DEV.common : BASE.API_PROD.common) + "/common"

// 文件上传接口
export const uploadFile = commonUrl+ "/upload/file";
// Websocket
export const ws = managerUrl + "/ws";

//获取所有city
export const getAllCity = (params) => {
  return getRequest(commonUrl + '/region/allCity', params)
}
...
export const initCaptcha = (params) => {
  return getRequestWithNoToken(commonUrl+"/captcha/init", params);
};
// 发送登录短信验证码
export const sendLoginSms = (mobile, params) => {
  return getRequest(commonUrl+`/captcha/sendLoginSms/${mobile}`, params);
};
// 发送注册短信验证码
export const sendRegistSms = (mobile, params) => {
  return getRequest(commonUrl+`/captcha/sendRegistSms/${mobile}`, params);
};
// 发送重置密码短信验证码
export const sendResetSms = (mobile, params) => {
  return getRequest(commonUrl+`/captcha/sendResetSms/${mobile}`, params);
};
// 发送修改绑定手机短信验证码
export const sendEditMobileSms = (mobile, params) => {
  return getRequest(commonUrl+`/captcha/sendEditMobileSms/${mobile}`, params);
};
// 通过手机重置密码
...
export const ipInfo = (params) => {
  return getRequest(commonUrl+"/ip/info", params);
};
// 个人中心编辑
...
export const getFileListData = (params) => {
  return getRequest(commonUrl+"/file", params);
};

// 重命名文件
export const renameFile = (params) => {
  return postRequest(commonUrl+"/file/rename", params);
};
// 删除文件
export const deleteFile = (ids, params) => {
  return deleteRequest(commonUrl+`/file/delete/${ids}`, params);
};
// 下载文件
...
export const base64Upload = (params) => {
  return postRequest(commonUrl+"/upload/file", params);
};
...
export const getFileDirectory = () => {
  return getRequest(commonUrl+`/resource/fileDirectory`);
};

// 添加文件目录
export const addFileDirectory = (params) => {
  return postRequestWithNoForm(commonUrl+`/resource/fileDirectory`,params);
};


// 修改文件目录
export const updateFileDirectory = (params) => {
  return putRequestWithNoForm(commonUrl+`/resource/fileDirectory`,params);
};


// 删除文件目录
export const delFileDirectory = (id) => {
  return deleteRequest(commonUrl+`/resource/fileDirectory/${id}`);
};
