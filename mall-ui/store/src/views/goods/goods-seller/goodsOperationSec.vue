<template>
  <div>
        ...
        <div class="base-info-item">
          ...
          <div class="form-item-view">
            ...
            >
              ...
              >
                <Button
                  @click="handleCLickImg('goodsGalleryFiles')"
                  type="primary"
                  >上传图片</Button
                >
              </div>
            ...
            >
              <div class="goods-video">
                ...
                <Upload
                  ...
                  type="drag"
                >
                  <div class="text" v-if="!loadingVideo">
                        ...
                        <Card :bordered="true" class="ivu-card-body">
                          ...
                          <div class="sku-val">
                            ...
                            <Form :model="item" class="flex">
                              ...
                              >
                                ...
                                >
                                  ...
                                  <a
                                    ...
                                    v-if="val.value && val.value !== ''"
                                  >
                                    <Icon
                                      size="15"
                                      type="md-trash"
                                      @click="
                                        handleCloseSkuValue(val, index, item)
                                      "
                                    ></Icon>
                                  </a>
                                </div>
                                <div
                                  v-if="$index === 0 && openImage"
                                  style="margin-top: 10px"
                                >
                                  <vuedraggable
                                    :animation="200"
                                  ...
                                  >
                                    <div
                                      v-for="(img, __index) in val.images"
                                      :key="__index"
                              ...
                              <FormItem
                                v-if="
                                  item.spec_values.length < 10 &&
                                  item.spec_values.length >= 1 &&
                                  item.spec_values[0].value !== ''
                                "
                                class="sku-item-content-val flex"
                                label=""
                                style="line-height: 32px"
          ...
          <div v-if="showContent" class="form-item-view">
            <div>
              <FormItem :label="contentImage" class="form-item-view-el">
                <!-- {{item.url}} -->
                <div
                  v-for="(item, index) in listImages.images"
                  :key="index"
          ...
          <div class="form-item-view">
            ...
            >
              ...
              <div class="promise-intro-btn">
                <span class="fontCol" @click="promiseIntroEditor"
                  >将PC商品描述同步到移动端描述
                </span>
              </div>
            </FormItem>
          ...
          <div v-if="baseInfoForm.goodsType != 'VIRTUAL_GOODS'">
            ...
            <div class="form-item-view">
              <FormItem
                ...
                prop="release"
              >
                <RadioGroup v-model="baseInfoForm.release" button-style="solid">
    ...
    <div class="footer btnBox">
      <ButtonGroup>
        <Button type="primary" @click="pre" class="mr_20">上一步</Button>
...
export default {
  ...
  methods: {
    ...
    confirmUrls() {
      if (
        this.selectedImage &&
        this.selectedFormBtnName == "selectedSkuImages"
      ) {
    ...
    getGoodsBrandList(type) {
      API_GOODS.getCategoryBrandListDataSeller(this.categoryId)
        .then((response) => {
    ...
    GET_GoodsParams() {
      ...
      API_GOODS.getCategoryParamsListDataSeller(this.categoryId).then(
        (response) => {
          ...
          this.goodsParams = response.result;
    ...
    skuValueChange(val, index, item, $index) {
      ...
      if (this.zz(0, val.value) > 20) {
        ...
        this.skuInfo[$index].spec_values[index].value = this.countCharacters(
          val.value,
          10
        );
        this.$forceUpdate(); // 调用该方法会触发组件的重新渲染
    ...
    checkSkuVal(val) {
      if (val.value === "") {
        ...
        this.skuInfo[skuIndex] &&
          (this.skuInfo[skuIndex].spec_values = this.skuInfo[
            skuIndex
          ].spec_values.filter((i) => i.value !== ""));
    ...
    specIterator(result, spec, skus) {
      ...
      if (spec.length > 0) {
        ...
        //删除数组的第一项元素，返回被删除的元素， 修改原数组
        cloneTemp.shift();
        //向skuSecSet添加规格，后边根据此set中的规格生成sku表格
        this.skuSecSet.add(spec[0].name);

        spec[0].spec_values.forEach((specItem) => {
          let index = this.skuIndex;
          // alert(index+" "+spec[0].name+" "+specItem.value)
          if (table[index]) {
            table[index][spec[0].name] = specItem.value;
            //upSkuList中记录了每次递归遍历元素的上个元素，用于生成sku表格
            if (!this.upSkuList[spec[0].name]) {
              this.upSkuList[spec[0].name] = [];
            }
            this.upSkuList[spec[0].name].push(specItem.value);
            ...
            let obj = {
              ...
              [spec[0].name]: skus[index][spec[0].name]
                ? skus[index][spec[0].name]
                : specItem.value,
              images:
            ...
            this.upSkuList[spec[0].name].push(specItem.value);
            //根据skuSecSet中的规格并从upSkuList中遍历填充sku表格
            this.skuSecSet.forEach((item) => {
              if (this.upSkuList[item]) {
                if (!table[index][item]) {
                  table[index][item] =
                    this.upSkuList[item][this.upSkuList[item].length - 1];
                }
              }
            });
          }
        ...
        API_GOODS.getGoodsSpecInfoSeller(categoryId).then((res) => {
          if (res.result.length) {
            res.result.forEach((e, index) => {
              ...
              console.log("skuVals=" + this.skuVals);
              //根据商品分类关联的规格信息在添加商品时自动生成sku信息
              let skuSpec = {
                name: e.specName,
                spec_id: e.specId,
                spec_values: [],
              };
              vals.forEach((item, index2) => {
                skuSpec.spec_values.push({
                  id: index2,
                  value: item,
                  images: [],
                });
              });
              this.skuInfo.push(skuSpec);

              // this.newSkuValues[index]=Array.from(new Set(vals))
              // this.addSpec(index,{name:e.specName,spec_values:this.newSkuValues[index]})
            });
      ...
      this.$refs["baseInfoForm"].validate((valid) => {
        if (valid) {
          ...
          if (submit.goodsGalleryFiles.length > 0) {
            submit.goodsGalleryList = submit.goodsGalleryFiles.map((i) => i);
          }
          /** 参数校验 **/
  ...
  mounted() {
    ...
    if (this.$route.query.id || this.$route.query.draftId) {
      ...
      if (this.firstData.tempId) {
        ...
        // let skuList=[];
        // this.skuData.forEach((specName, index) => {
        //   alert(specName)
        //   this.skuVals[index].forEach((specValue,index2) =>{
        //     alert(specName+"  "+specValue)
        //     skuList.push(
        //       {
        //         "id": "1765653920287346690",
        //         "goodsId": "1765653919972773889",
        //         "simpleSpecs": " 红 6",
        //         "freightTemplateId": "1765641638744477698",
        //         "promotionFlag": null,
        //         "promotionPrice": null,
        //         "goodsName": "荣耀70 红 6",
        //         "sn": "100101",
        //         "brandId": "1465306502990786561",
        ...
        //         "quantity": 999,
        //         "grade": 100.0,
        //         "thumbnail": "http://localhost:9000/mall/STORE/1376417684140326912/undefined/31d0a78e77c44570a41f0dc7d0dc39cf.png",
        //         "big": "http://localhost:9000/mall/STORE/1376417684140326912/undefined/31d0a78e77c44570a41f0dc7d0dc39cf.png",
        //         "small": "http://localhost:9000/mall/STORE/1376417684140326912/undefined/31d0a78e77c44570a41f0dc7d0dc39cf.png",
        //         "original": "http://localhost:9000/mall/STORE/1376417684140326912/undefined/31d0a78e77c44570a41f0dc7d0dc39cf.png",
        //         "storeCategoryPath": "1420665580290355201",
        //         "commentNum": null,
