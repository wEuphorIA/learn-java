<template>
  <div>
    <!-- 选择商品类型 -->
    <Modal title="模板导入" v-model="selectGoodsType" width="420" class="goodsSelect">
      <div v-if="!showGoodsTemplates">
        <div
      ...
      <div v-else>
        <h2 class="tit">
          <Button @click="showGoodsTemplates = !showGoodsTemplates"
            >返回</Button
          >
        </h2>
    ...
    <div class="content-goods-publish">
      <div class="goods-category">
        ...
        <ul v-if="categoryListLevel3.length > 0" class="listLast">
          <li
      ...
      <p class="current-goods-category">
        ...
        <span v-show="category[1].name"
          ><Icon type="ios-arrow-forward" /> {{ category[1].name }}</span
        >
        <span v-show="category[2].name"
          ><Icon type="ios-arrow-forward" /> {{ category[2].name }}</span
        >
      </p>
    ...
    <div class="footer btnBox">
      <ButtonGroup>
        <Button type="primary" class="mr_20" @click="selectGoodsType = true"
          >商品类型</Button
        >
...
export default {
  data() {
    return {
      ...
      // 商品类型
      goodsType: "PHYSICAL_GOODS", //默认实物商品
      /** 1级分类列表*/
      categoryListLevel1: [],
      /** 2级分类列表*/
      categoryListLevel2: [],
