  ...
  <div class="search">
        ...
        <Row class="operation padding-row">
          ...
          <Dropdown @on-click="handleDropdown">
            ...
            <DropdownMenu slot="list">
              ...
              <DropdownItem name="batchShipTemplate"
                >批量设置物流模板</DropdownItem
              >
            </DropdownMenu>
          </Dropdown>
        </Row>
      ...
      <Tabs value="updateStock" v-model="updateStockType">
        <TabPane label="手动规格更新" name="updateStock">
          <Table
            ...
            :data="stockList"
            border
            v-if="updateStockModalVisible"
          ></Table>
        </TabPane>
        <TabPane label="批量规格更新" name="stockAll">
          <Input
            type="number"
            v-model="stockAllUpdate"
            class="mt_20"
            placeholder="统一规格修改"
          />
        </TabPane>
    ...
    <Modal title="导入商品信息" v-model="importModal" :mask-closable="false">
      <div style="text-align: center">
        ...
        >
          <div>
            <Icon type="ios-cloud-upload" class="cloudUpload"></Icon>
            <!-- <h2>选择或拖拽文件上传</h2> -->
          </div>
          <Spin fix v-if="spinShow"></Spin>
        </Upload>
        <Button @click="exportGoods" type="text" style="color: red"
          >下载导入模板</Button
        >
      </div>
      <div slot="footer">
        <Button type="primary" @click="importModal = false">确定</Button>
      </div>
    </Modal>
  </div>
</template>
    ...
    return {
      ...
      updateStockColumns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            if (params.row.authFlag == "TOBEAUDITED") {
              return h("span", { class: "statusDot statusBlur" }, "待审核");
            } else if (params.row.authFlag == "PASS") {
              return h("span", { class: "statusDot statusRreen" }, "通过");
            } else if (params.row.authFlag == "REFUSE") {
              return h("span", { class: "statusDot statusRed" }, "审核拒绝");
            }
          },
        },
        // {
        ...
        // },
        {
          ...
          minWidth: 120,
          // render: (h, params) => {
          //   return h("div", {}, JSON.parse(JSON.stringify(params.row.quantity)));
          // },
        },
        {
          title: "入库/出库",
          ...
          render: (h, params) => {
            ...
            return h("InputNumber", {
              ...
              on: {
                "on-change": (event) => {
                  const num = params.row.quantity;
                  if (event < 0 && event < -num) {
                    vm.stockList[params.index].newQuantity = -num;
                  } else {
                    vm.stockList[params.index].newQuantity =
        ...
        },
        {
          ...
          render: (h, params) => {
            if (params.row.salesModel === "RETAIL") {
              return h("span", { class: "statusDot statusOrange" }, "零售");
            } else if (params.row.salesModel === "WHOLESALE") {
              return h("span", { class: "statusDot statusRreen" }, "批发");
            } else {
              return h("span", { class: "statusDot statusGray" }, "其他类型");
            }
          },
        },
        {
          ...
          render: (h, params) => {
            if (params.row.goodsType === "PHYSICAL_GOODS") {
              return h("span", { class: "statusDot statusBlur" }, "实物商品");
            } else if (params.row.goodsType === "VIRTUAL_GOODS") {
              return h("span", { class: "statusDot statusOrange" }, "虚拟商品");
            } else {
              return h("span", { class: "statusDot statusBlur" }, "电子卡券");
            }
          },
        },
        {
        ...
        },
        {
          ...
          render: (h, params) => {
            if (params.row.marketEnable == "DOWN") {
              return h("span", { class: "statusDot statusRed" }, "下架");
            } else if (params.row.marketEnable == "UPPER") {
              return h("span", { class: "statusDot statusRreen" }, "上架");
            }
          },
        },
        {
      ...
      if (
        ...
        let updateStockList = this.stockList.map((i) => {
          let j = {
            skuId: i.id,
            goodsId: i.goodsId,
            quantity: i.newQuantity ? i.newQuantity : i.quantity,
          };
