  ...
  <div class="search liveGoods">
    <Card>
      ...
      >
        ...
        <Button
          @click="getLiveGoodsMethods('clear')"
          type="primary"
          class="search-btn"
          icon="ios-search"
          >搜索</Button
      ...
      <div class="boxBg boxPad m24">
        <h4 v-if="!reviewed" class="auditTit">
        ...
        <div style="position: relative">
          ...
          >
            ...
            <template slot-scope="{ row, index }" class="price" slot="price">
              ...
              <RadioGroup
                v-if="params.auditStatus == 99"
                @on-change="changeRadio(row, 'priceType')"
              ...
              >
                <div class="price-item">
                  ...
                  <InputNumber
                    :min="0.1"
                    v-if="liveGoodsData[index].priceType == 1"
                    style="width: 100px"
                    v-model="liveGoodsData[index].price"
                  ></InputNumber>
                </div>
                <div class="price-item">
                  <Radio :label="2">区间价:</Radio>
                  <span v-if="liveGoodsData[index].priceType == 2">
                    <InputNumber
                      :min="0.1"
                      style="width: 100px"
                      v-model="liveGoodsData[index].price"
                    ...
                    <InputNumber
                      :min="0.1"
                      style="width: 100px"
                      v-model="liveGoodsData[index].price2"
                    />
                  </span>
                </div>
                <div class="price-item">
                  <Radio :label="3">折扣价:</Radio>
                  <span v-if="liveGoodsData[index].priceType == 3"
                    >原价<InputNumber
                      :min="0.1"
                      style="width: 100px"
                      v-model="liveGoodsData[index].price"
              ...
              <div v-else>
                <div v-if="row.priceType == 1">
                  <priceColorScheme :value="row.price" :color="$mainColor" />
                </div>
                <div class="flex" v-if="row.priceType == 2">
                  <priceColorScheme
                    :value="row.price"
                    :color="$mainColor"
                  />至<priceColorScheme
                ...
                <div class="flex" v-if="row.priceType == 3">
                  <priceColorScheme
                    :value="row.price2"
                    :color="$mainColor"
            ...
            <template slot-scope="{ row, index }" slot="action">
              <Button
                ...
                @click="
                  () => {
                    liveGoodsData.splice(index, 1);
                  }
                "
                >删除</Button
              >
              <Button
                v-if="params.auditStatus != 99 && !reviewed"
                ghost
                type="primary"
                @click="
                  () => {
              ...
              <Button
                v-if="reviewed"
                :type="row.___selected ? 'primary' : 'default'"
                @click="selectedLiveGoods(row, index)"
                >{{ row.___selected ? "已" : "" }}选择</Button
              >
            </template>
          </Table>
          <div class="flex">
    ...
    <div v-if="params.auditStatus == 99" class="submit">
      <Button type="primary" :loading="saveGoodsLoading" @click="saveLiveGoods"
        >保存商品</Button
      >
    </div>
  </div>
