<template>
  <div class="search logistics">
    ...
    <Modal
      ...
      class="logiModal"
    >
      <h3 class="tit">是否需要电子面单？</h3>

      <ButtonGroup style="margin-bottom: 10px">
        <Button
          :type="faceSheetForm.faceSheetFlag ? 'primary' : 'default'"
          @click="faceSheetForm.faceSheetFlag = true"
          >需要</Button
        >
        <Button
          :type="!faceSheetForm.faceSheetFlag ? 'primary' : 'default'"
          @click="faceSheetForm.faceSheetFlag = false"
          >不需要</Button
        >
      </ButtonGroup>
      <Card v-if="openText" class="modalStyle">
        <h3 class="tit">请输入详细信息</h3>
        <Form
          ref="formValidate"
          :label-width="150"
        ...
        >
          ...
          <div style="width: 100%; text-align: center">
            <a style="padding-right: 20px" @click="frontDownload('use')"
              >使用说明</a
            >
        ...
        },
        {
          ...
          render: (h, params) => {
            if (!params.row.selected) {
              return h("div", [
                h("span", { class: "statusDot statusGray" }, "关闭"),
              ]);
            } else {
              return h("div", [
                h("span", { class: "statusDot statusRreen" }, "开启"),
              ]);
            }
          },
        },
  ...
  methods: {
    ...
    submit() {
      if (!this.row.selected) {
        ...
        API_Shop.editChecked(this.logisticsId, this.faceSheetForm).then(
          (res) => {
            if (res.success) {
