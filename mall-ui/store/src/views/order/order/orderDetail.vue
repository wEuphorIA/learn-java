<template>
  <div class="search base-detail orderDetail">
    <Card class="mt_24">
      <div class="boxBg">
        <div class="headBtn">
          <div class="right">
            ...
            >
            <Button
              v-if="allowOperation.showLogistics || orderPackage.length > 0"
              @click="checkLogistics"
              type="primary"
              class="mr_20"
              >查看物流</Button
            >
            <Button @click="orderLogModal = true" type="primary" class="mr_20"
              >订单日志</Button
            >
            <Button @click="printOrder" type="primary" class="mr_20">打印发货单</Button>
            <Button v-if="allowOperation.take" @click="orderTake" type="primary" class="mr_20"
              >订单核销</Button
            >

            <Button v-if="allowOperation.ship" @click="groupShip" type="primary" class="mr_20"
              >分包裹发货</Button
            >
          ...
          <Row>
            <Col span="12">
              ...
              <div class="div-item">
                ...
                <div class="div-item-right">
                  <span
                    class="statusDot statusRed"
                    v-if="orderInfo.orderStatusValue === '未付款'"
                    >{{ orderInfo.orderStatusValue }}</span
                  >
                  <span
                    class="statusDot statusRreen"
                    v-if="orderInfo.orderStatusValue === '已付款'"
                    >{{ orderInfo.orderStatusValue }}</span
                  >
                  <span
                    class="statusDot statusOrange"
                    v-if="orderInfo.orderStatusValue === '待发货'"
                    >{{ orderInfo.orderStatusValue }}</span
                  >
                  <span
                    class="statusDot statusBlur"
                    v-if="orderInfo.orderStatusValue === '待自提'"
                    >{{ orderInfo.orderStatusValue }}</span
                  >
                  <span
                    class="statusDot statusRreen"
                    v-if="orderInfo.orderStatusValue === '已发货'"
                    >{{ orderInfo.orderStatusValue }}</span
                  >
                  <span
                    class="statusDot statusGray"
                    v-if="orderInfo.orderStatusValue === '已完成'"
                    >{{ orderInfo.orderStatusValue }}</span
                  >
                  <span
                    class="statusDot statusOrange"
                    v-if="orderInfo.orderStatusValue === '待核验'"
                    >{{ orderInfo.orderStatusValue }}</span
                  >
                  <span
                    class="statusDot statusGray"
                    v-if="orderInfo.orderStatusValue === '已取消'"
                    >{{ orderInfo.orderStatusValue }}</span
                  >
                </div>
              </div>
              ...
              <template
                v-if="orderInfo.order.needReceipt == true && orderInfo.receipt"
              >
                <div class="div-item">
            ...
            <Col span="12">
              <div
                class="div-item"
                v-if="orderInfo.order.deliveryMethod != 'SELF_PICK_UP'"
              >
              ...
              <div class="div-item">
                ...
                <div class="div-item-right">
                  {{ orderInfo.order.remark ? orderInfo.order.remark : "--" }}
                </div>
              </div>

              <div
                class="div-item"
                v-if="orderInfo.order.orderType != 'VIRTUAL'"
              >
              ...
              <li
                v-if="
                  orderInfo.order.priceDetailDTO.discountPriceDetail !=
                    undefined &&
                  orderInfo.order.priceDetailDTO.discountPriceDetail &&
                  orderInfo.order.priceDetailDTO.discountPriceDetail != null &&
                  orderInfo.order.priceDetailDTO.discountPriceDetail != ''
                "
              >
                <div class="label">
      ...
      <div style="max-height: 500px; overflow-y: auto; overflow-x: hidden">
        <div id="printInfo">
          ...
          <Row>
            <Col span="24">
              <p class="lineH30 f14">
                收货地址：{{ orderInfo.order.consigneeAddressPath
                }}{{ orderInfo.order.consigneeDetail }}
              </p>
            </Col>
          </Row>
    ...
    >
      <div>
        <Form
          ref="groupOrderDeliveryForm"
          :model="groupOrderDeliveryForm"
          :label-width="90"
          :rules="groupOrderDeliverFormValidate"
          style="position: relative"
        >
          <FormItem label="物流公司" prop="logisticsId">
            <Select
      ...
      <Table
        @on-select="selectGroupShipGoodsMethods"
        @on-selection-change="selectGroupShipGoodsMethods"
        @on-select-all="selectGroupShipGoodsMethods"
        :data="data"
      ...
      >
        ...
        <template slot="numSlot" slot-scope="{ row, index }">
          <InputNumber
            :min="0"
            :max="row.___num - row.deliverNumber"
            v-model="data[index].canNum"
          >
          </InputNumber>
        </template>
      </Table>
      ...
      groupShipColumns: [
        ...
        { title: "数量", key: "num", slot: "numSlot", width: 120 },
        {
          ...
          render: (h, params) => {
            return h(
              "div",
              params.row.deliverNumber ? params.row.deliverNumber : 0
            );
          },
        },
        {
