<template>
  <div class="wrapper cartBox">
    <u-navbar :is-back="false" class="cartNav">
      <view slot:left class="clearCart" @click="handleClear()">清空购物车</view>
      <view class="title">购物车</view>

      <div slot="right">
        <div class="light-color edit" @click="isEdit = !isEdit">
          {{ !isEdit ? '编辑' : '完成' }}
        </div>
      </div>
    </u-navbar>
    <view :style="{ paddingTop: capsuleBottom + 'px' }">
      ...
      <div class="content">
        <div
          ...
          v-for="(item, index) in cartDetail.cartList"
          :key="index"
        >
          <view class="tab">
            <view class="store-line">
              ...
              <span
                class="store-name wes store-line-desc"
                @click.stop="navigateToStore(item)"
                >{{ item.storeName }}</span
              >
              <u-icon
            ...
            <view
              class="right-col"
              v-if="item.canReceiveCoupon"
              @click="navigateToCoupon(item)"
            >
              <span>领劵</span>
            </view>
          </view>
          ...
          >
            ...
            <view class="goods-row" :class="{ invalid: isInvalid(skuItem) }">
              <view class="goods-config">
                <view>
                  <u-checkbox-group
                    v-if="skuItem.invalid == 0 && !skuItem.errorMessage"
                  >
                    <!-- #ifndef MP-WEIXIN -->
                    <u-checkbox
                      shape="circle"
                  ...
                  <span class="invalid" v-else style="font-size: 24rpx"
                    >失效</span
                  >
                </view>
                <u-image
              ...
              <view class="goods-content">
                ...
                <!-- 规格 -->
                <p class="sp-type" style="width: 300rpx">
                  {{ skuItem.goodsSku.simpleSpecs }}
                </p>
                <p
                  class="sp-type"
                  v-if="skuItem.goodsSku.salesModel == 'WHOLESALE'"
                >
                  批发商品
                </p>
                <div class="sp-number">
                  <view class="sp-price">
                    ...
                    <div v-if="skuItem.priceDetailDTO.couponPrice > 0">
                      <div class="main-color">
                        ￥<span>{{
                          $options.filters.goodsFormatPrice(
                            skuItem.priceDetailDTO.billPrice
                          )[0]
                        }}</span>
                        <span
                          >.{{
                  ...
                  <!-- 如果当有促销并且促销是 限时抢购 -->
                  <!-- promotions -->
                  <div class="promotions-list" v-if="skuItem.promotionMap">
                    <div
                      class="promotions-item-seckill"
                      v-if="getPromotion(skuItem).includes('SECKILL')"
                    >
                      距秒杀结束:
                      <u-count-down
                        show-border
                  ...
                  <div
                    ...
                    v-if="!skuItem.checked && skuItem.errorMessage"
                  >
          ...
          <span class="price">
            <div class="prices">
              <div class="fullPrice">
                <span
                  class="number"
                  v-if="cartDetail && cartDetail.priceDetailDTO"
                >
                  总计:
                  <span
                    >¥{{
                      $options.filters.goodsFormatPrice(
                        cartDetail.priceDetailDTO.flowPrice
                      )[0]
                    }}</span
                  >.<span>{{
                    $options.filters.goodsFormatPrice(
                ...
                <span class="number" v-else>总计:<span>0.00</span></span>
                <span
                  class="main-color ml_20"
                  v-if="cartDetail && cartDetail.canUseCoupons.length > 0"
                  @click="GET_Discount()"
                >
                  <span>使用优惠券</span>
                </span>
              ...
              <div
                v-if="
                  cartDetail.cartList &&
                  cartDetail.cartList.length != 0 &&
                  cartDetail.priceDetailDTO &&
                  cartDetail.priceDetailDTO.couponPrice != 0
                "
                class="discountPrice"
              >
                <span
                  >优惠减:￥{{
                    (cartDetail.priceDetailDTO.goodsPrice -
                      cartDetail.priceDetailDTO.flowPrice)
                      | unitPrice
                  }}
                </span>
                <span class="discount-details" @click="discountDetails"
                  >优惠明细</span
                >
              </div>
            </div>
          ...
          <div class="discount-list">
            ...
            <div class="discount-way">
              <div class="discount-item" v-if="cartDetail.priceDetailDTO">
                ...
                <span
                  >￥{{
                    cartDetail.priceDetailDTO.goodsPrice | unitPrice
                  }}</span
                >
              </div>
              <div class="discount-item" v-if="cartDetail.priceDetailDTO">
                ...
                <span
                  >-￥{{
                    cartDetail.priceDetailDTO.couponPrice | unitPrice
                  }}</span
                >
              </div>
              <div class="discount-item" v-if="cartDetail.priceDetailDTO">
                ...
                <span
                  >-￥{{
                    cartDetail.priceDetailDTO.discountPrice | unitPrice
                  }}</span
                >
              </div>
            </div>
...
export default {
  ...
  onLoad() {
    uni.getSystemInfo({
      success: () => {
        this.capsuleBottom = uni.getMenuButtonBoundingClientRect().bottom;
      },
    });
  },
  /**
  ...
  methods: {
    ...
    getCountDownTime(val) {
      if (val.promotionMap) {
        ...
        return (
          val.promotionMap[key].endTime / 1000 - new Date().getTime() / 1000
        );
      }
    },
    ...
    handleClear() {
      ...
      if (delGoodsData && delGoodsData.length > 0) {
        // 执行删除
        API_Trade.deleteAllSkuItem(delGoodsData).then((res) => {
          if (res.data.success) {
            uni.showToast({
              title: '删除成功!',
              icon: 'none',
        ...
        API_Trade.getCarts()
          .then((result) => {
            ...
            if (result.data.success) {
              ...
              this.$store.state.canUseCoupons =
                result.data.result.canUseCoupons;
              this.$store.state.cantUseCoupons =
                result.data.result.cantUseCoupons;
              let checkOuted = true;
              for (let i = 0; i < this.cartDetail.cartList.length; i++) {
                ...
                item.skuList &&
                  item.skuList.forEach((sku) => {
                    ...
                    if (
                      sku.promotionMap &&
                      Object.keys(sku.promotionMap).length != 0
                    ) {
              ...
              uni.stopPullDownRefresh();
              console.log(this.cartDetail, 11111111);
            }
          })
          .catch((err) => {
            this.loading = false;
    ...
    GET_Discount() {
      ...
      let skus = [];
      let selectedCoupon = [];
      if (this.cartDetail.platformCoupon)
        selectedCoupon.push(this.cartDetail.platformCoupon.memberCoupon.id);
      if (
        this.cartDetail.storeCoupons &&
        Object.keys(this.cartDetail.storeCoupons)[0]
      ) {
        let storeMemberCouponsId = Object.keys(this.cartDetail.storeCoupons)[0];
        let storeCouponId =
          this.cartDetail.storeCoupons[storeMemberCouponsId].memberCoupon.id;
      ...
      uni.setStorage({
        key: 'totalPrice',
        data: this.cartDetail.priceDetailDTO.goodsPrice,
      });
      this.navigateTo(
        `/pages/cart/coupon/index?way=CART&storeId=${store}&skuId=${skus}&selectedCoupon=${selectedCoupon}`
      );
    },
  },
};
...
.wrapper {
  ...
  /deep/ .u-checkbox .u-checkbox__icon-wrap {
    background: #fff !important;
    border: 1px solid #bfbfbf !important;
  }
  /deep/ .u-checkbox .u-checkbox__icon-wrap--checked {
