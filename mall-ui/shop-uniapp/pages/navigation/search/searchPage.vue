      ...
      <view class="search-keyword" v-if="!isShowSeachGoods">
        ...
        <block v-for="(row, index) in keywordList" :key="index">
          <view class="keyword-entry" hover-class="keyword-entry-tap">
            <view
              class="keyword-text"
              @tap.stop="doSearch(keywordList[index].words)"
        ...
        <div class="keyword-box" v-show="!isShowKeywordList">
          <view class="keyword-block" v-if="oldKeywordList.length > 0">
            <view class="keyword-list-header"
              ...
              ><view class="delectIcon" @tap="oldDelete"></view
            ></view>
          ...
          <view class="keyword-block add1" v-if="hotKeywordList.length > 0">
            <view class="keyword-list-header"
              ><view class="u-tips"
                ><view class="hotIcon"></view>热门搜索</view
              ></view
            >
      ...
      <view class="sort-box">
          ...
          <view class="sort-item">
            ...
            <view class="flex" v-if="sortData.brands">
              <view
                ...
                :key="brandsIndex"
                v-for="(brand, brandsIndex) in sortData.brands"
        ...
        <view class="sort-list" v-if="sortData.paramOptions">
          <view
            class="sort-item"
            :key="paramIndex"
            v-for="(param, paramIndex) in sortData.paramOptions"
          >
...
export default {
    ...
    return {
      ...
      records: {
        ...
        brands: [
          {
            name: '小米',
            value: '1349563815406887012',
            url: 'https://yjy-xzbjzfw-oss.oss-cn-hangzhou.aliyuncs.com/MANAGER/1782333747685765121/ec2d0f02ac1246e8adc470c1e21049ce.png',
            otherOptions: null,
          },
          {
            name: '荣耀',
            value: '1465306502990786561',
            url: 'https://yjy-xzbjzfw-oss.oss-cn-hangzhou.aliyuncs.com/MANAGER/1782333747685765121/4cd71eb15c8c40fa97d879f62cf5c5a4.jpg',
            otherOptions: null,
          },
        ],
        paramOptions: [
          { key: '内存', values: ['128G', '256G', '256'] },
          { key: '颜色', values: ['红色', '绿色', '黑色', '白色'] },
          { key: '4G网络', values: ['4G FDD-LTE'] },
          { key: '5G网络', values: ['支持5G'] },
          { key: '充电接口', values: ['Type-C'] },
        ],
      },
    };
  },
  ...
  onLoad(val) {
    ...
    this.loadData();
    console.log(this.isShowSeachGoods);
    uni.getSystemInfo({
      success: () => {
        this.capsuleBottom = uni.getMenuButtonBoundingClientRect().bottom + 34;
      },
    });
  },
  components: {
  ...
  methods: {
    ...
    handleSort(val, index, type, parent) {
      ...
      if (type === 'categoryId' && this.selectedWay.categoryId.length > 0) {
        this.selectedWay.categoryId.forEach((w, i) => {
          if (w.name !== val.name) {
            this.selectedWay.categoryId.splice(i, 1);
          }
        });
      }
      if (type == 'categoryId') {
        ...
        if (
          type === 'brand' &&
          this.sortData.brands &&
          this.sortData.brands.length > 0
        ) {
          this.sortData.brands.forEach((br, bri) => {
            if (val.name === br.name) {
              br.__selected = true;
            } else {
              br.__selected = false;
        ...
        this.sortData.paramOptions.forEach((ele, ei) => {
          if (ele.key === val.parent) {
            ele.__selected = true;
            ele.values.forEach((obj) => {
      ...
      this.selectedWay.brand.forEach((p, pi) => {
        if (!p.__selected) {
          this.selectedWay.brand.splice(pi, 1);
        }
      });
      let brandsItems = [
        ...new Map(
          this.selectedWay.brand.map((item) => [item.parent, item])
        ).values(),
      ];
      this.selectedWay.brand = brandsItems;
      this.selectedWay.prop.forEach((p, pi) => {
        if (!p.__selected) {
          this.selectedWay.prop.splice(pi, 1);
        }
      });
      let uniqueItems = [
        ...new Map(
          this.selectedWay.prop.map((item) => [item.parent, item])
        ).values(),
      ];
      this.selectedWay.prop = uniqueItems;
      console.log(this.selectedWay, 45987);
    },
          ...
          for (let item of Object.keys(res.data.result)) {
            res.data.result[item].forEach((child) => {
              ...
              if (child.values) {
                ...
                if (this.selectedWay.prop && this.selectedWay.prop.length > 0) {
                  this.selectedWay.prop.forEach((p) => {
              ...
              if (child.name) {
                this.selectedWay.brand.forEach((b) => {
                  if (child.name === b.name) {
                    child.__selected = true;
                  }
                });
                this.selectedWay.categoryId.forEach((b) => {
                  if (child.name === b.name) {
                    child.__selected = true;
                  }
                });
    ...
    async loadData(type, loading) {
      ...
      if (
        goodsList.data.result.records &&
        goodsList.data.result.records.length < 10
      ) {
        this.loadingType = 'noMore';
      }
      this.goodsList.push(...goodsList.data.result.records);
      console.log(this.goodsList, 5896);
