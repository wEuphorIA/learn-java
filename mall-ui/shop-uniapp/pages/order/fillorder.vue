<template>
  <div class="wrapper">
    <u-navbar :is-back="true" title="填写订单"> </u-navbar>
      ...
      <view class="orderCon">
        ...
        <div
          ...
          v-if="
            ...
            orderMessage.cartTypeEnum != 'VIRTUAL'
          "
        >
          <div class="user-box flex">
            ...
            <div class="addressCon">
              ...
              <u-icon name="arrow-right" style="color: #878787"></u-icon>
            </div>
          </div>
        </div>
        <!-- 选择自提点 -->
        ...
        <view class="group-box" v-if="isAssemble">
          <view class="group-title">
            ...
            <span v-else
              >为你加入仅差<span>{{
                routerVal.parentOrder.toBeGroupedNum
              }}</span
              >人的团购买</span
            >
          </view>
        ...
        >
          <div v-if="item.checked">
            ...
            <div class="promotionNotice" v-if="item.promotionNotice">
            ...
            <u-row>
              ...
              >
                {{
                  shippingText!==''?shippingMethod.find((e) => {
        ...
        <div class="box6 mp-iphonex-bottom" v-if="orderMessage.priceDetailDTO">
          <div class="tabbar-left">
            <!-- v-if="!orderMessage.priceDetailDTO.payPoint" -->
            <div class="number">
              <span>¥</span>
              <span class="price">{{
    ...
    createTradeFun() {
      ...
      this.$u.throttle(() => {
        if (this.shippingText === 'SELF_PICK_UP') {
          ...
          this.shippingText === 'LOGISTICS' &&
          this.orderMessage.cartTypeEnum !== 'VIRTUAL'
        ) {
          if (!this.address.id) {
            uni.showToast({
              title: '请选择地址',
        ...
        API_Trade.createTrade(submit).then((res) => {
          if (res.data.success) {
            ...
            // 如果当前价格为0跳转到订单列表
            console.log(this.orderMessage.priceDetailDTO.billPrice);
...
.box6 {
  ...
  box-shadow: 0 -3px 10px -3px #d6d6d6;
  padding-left: 30rpx;
  padding-right: 30rpx;
}
...
.user-box {
  ...
  .addressIcon {
    padding-right: 12rpx;
    .icon {
      background: url('@/static/icon/icon-dizhi2.png') no-repeat;
