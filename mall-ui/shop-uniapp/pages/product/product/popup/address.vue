      ...
      <view class="view-item" v-for="(item, index) in addressDetail" :key="index" @click="clickAddress(item)">
        <view class="view-box-checkbox">
          <view class="checkbox" :class="{ checked: item.isDefault }">
            <!-- <u-icon></u-icon> -->
            <view v-if="item.isDefault" :class="{ active: item.isDefault }"></view>
          </view>
        </view>
...
.checkbox {
  ...
  width: 42rpx;
  height: 42rpx;
  background: url('@/static/icon/icon-address.png') no-repeat;
  background-size: contain;
  &.checked{
    background: url('@/static/icon/icon-dizhi.png') no-repeat;
    background-size: contain;
  }
