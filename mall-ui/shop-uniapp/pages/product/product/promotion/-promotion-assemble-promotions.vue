            ...
            <div class="u-group-row">
              <view :span="8" class="showBox_L">
                <view class="u-group-flex">
                  <!-- 限时抢购 -->
                  <view
                    class="u-group-flex-left"
                    v-if="promotion.__key == 'SECKILL'"
                  >
                    <span
                  ...
                  >
                    <span class="u-group-flex-left-span">
                      <span
                        ...
                        >￥{{
                          $options.filters.goodsFormatPrice(
                            promotion.groupbuy_goods_vo.price
                          )[0]
                        }}.{{
                          $options.filters.goodsFormatPrice(
                            promotion.groupbuy_goods_vo.price
                    ...
                    <view class="u-group-flex">
                      <span
                        class="old-price"
                        v-if="
                          promotion.groupbuy_goods_vo.original_price !=
                          undefined
                        "
                        >￥{{
                          $options.filters.goodsFormatPrice(
                  ...
                  <!--  拼团右侧 -->
                  <view
                    class="u-group-flex-right"
                    v-if="promotion.__key == 'PINTUAN'"
                  >
                    <span class="group-bag"
                      >{{ promotion.requiredNum }}人拼团
                    </span>
                    <span class="group-bag"
                      >限购{{ promotion.limitNum }}件</span
                    >
                  </view>
                </view>
              </view>
        ...
        >
          <view class="group-wrapper">
            <div class="u-group-row">
              <view :span="8" class="showBox_L">
                <view class="u-group-flex">
                  ...
                  >
                    <span
                      class="u-group-flex-left-span"
                      v-if="goodsData.price != undefined"
                    >
                      ￥
                      <span class="flex-price">
                        {{
                    ...
                    <!-- <view class="u-group-flex" v-if="goodsData.price != undefined">
                      <span class="old-price"
                        >￥{{
                          $options.filters.goodsFormatPrice(goodsData.price)[0]
                        }}.{{
                          $options.filters.goodsFormatPrice(goodsData.price)[1]
                        }}</span
                      >
                      <view class="promotion">限时抢购</view>
                    </view> -->
              ...
              <view class="showBox_R">
                <u-tag :text="getIsTimer(goodsData)" size="mini" type="error" />
                <u-count-down
                  :hide-zero-day="true"
                  font-size="25"
                  color="#fff"
                  ...
                  :show-hours="true"
                  :show-minutes="true"
                  :timestamp="getCountTime(goodsData.startTime)"
                ></u-count-down>
              </view>
            </div>
          </view>
...
export default {
  ...
  watch: {
    res: {
      handler() {
        if (this.res.price !== undefined) {
          this.goodsData = this.res;
        } else {
  ...
  methods: {
    getCountDownTime(val) {
      let timeSimple = Number(val) / 1000;
      return timeSimple - new Date().getTime() / 1000;
    },
    getCountTime(val) {
      let timeSimple = new Date(val).getTime() / 1000;
      return timeSimple - new Date().getTime() / 1000;
    },
    getIsTimer(val) {
      var timestamp = new Date().getTime();
      if (timestamp < new Date(val.startTime).getTime()) {
