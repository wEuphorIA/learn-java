    ...
    >
                ...
                <view>
                  <view class="-goods-flex">
                        ...
                        <span v-if="wholesaleList.length">
                          ...
                          ><span class="price">{{
                            $options.filters.goodsFormatPrice(
                              wholesaleList[wholesaleList.length - 1].price
                            )[0]
                          }}</span
                          >.{{
                            $options.filters.goodsFormatPrice(
                    ...
                    <view class="-goods-price" v-else>
                      ...
                      <span v-else> ¥<span class="price">0 </span>.00 </span>
                    </view>
                    <view class="" @click="shutMask(1)">
                      <span v-if="PromotionList && emptyPromotion()"></span>
                      <PromotionLayout
                        v-else
                        @shutMasks="shutMask"
                        :res="PromotionList"
      ...
      <view class="page-bottom mp-iphonex-bottom" id="pageBottom">
        <view class="icon-btn">
          ...
          >
            <u-icon size="40" class="red" name="home-fill"></u-icon>
          ...
          <view class="icon-btn-item" @click="reluchToCart()">
            <u-icon size="40" name="storeping-cart"></u-icon>
        ...
        <view class="detail-btn" v-if="!isGroup && !takeDownFromSale">
          <view
            v-if="routerVal.type === undefined || routerVal.type === null"
...
export default {
  ...
  methods: {
    ...
    async init(id, goodsId, distributionId = '') {
      ...
      if (this.routerVal.type === 'ms') {
        this.PromotionList = {
          startTime: this.routerVal.time,
          endTime: this.routerVal.endTime,
          price: Number(this.routerVal.price),
          promotionPrice: Number(this.routerVal.promotionPrice),
        };
        this.isSeckill = true;
      }

      Object.keys(response.data.result.promotionMap).forEach((item) => {
        if (item) {
      ...
      // 轮播图
      this.imgList = this.goodsDetail.goodsGalleryList.filter(
        (i) => i.indexOf('"url":') === -1 && i.indexOf('"status":') === -1
      );
