...
public interface FailMsgMapper {
    ...
    @Insert("insert into fail_msg(id,exchange,routing_key,msg,reason,delay_msg_execute_time,next_fetch_time,create_time) values(#{domain.id}," +
            "#{domain.exchange},#{domain.routingKey},#{domain.msg},#{domain.reason},#{domain.delayMsgExecuteTime},#{domain.nextFetchTime},#{domain.createTime})")
    int insert(@Param("domain") FailMsg failMsg);

    @Select("select" + BASE_COLUMNS + "from fail_msg where id=#{id} limit 1")
    List<FailMsg> queryById(@Param("id") String id);
    ...
    void batchUpdateExecuteTime(@Param("time") Integer time, @Param("ids") List<String> ids);
